...
const escapeMd = (text) => text.replace(/\|/g, "\\|").replace(/[\r\n]+/g, " ");
export const formatInt = (n) => (n === null ? "–" : n.toLocaleString("en-US"));
/** Full days before `today` (today is partial and excluded), oldest first, zero-filled. */
export function lastDays(file, today, days) {
...
export function latest(file, key, today) {
    for (let i = file.daily.length - 1; i >= 0; i--) {
        const row = file.daily[i];
        if (row.date <= today && row[key] !== null)
            return row[key];
    }
    return null;
}
export function sparklineSvg(values, width = 400, height = 80) {
    const pad = 4;
    ...
    return (`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="views, last ${values.length} days, max ${max}">` +
        `<title>views, last ${values.length} days (max ${max})</title>` +
        `<rect width="${width}" height="${height}" fill="#0d1117" rx="6"/>${line}</svg>`);
}
export function badgeSvg(label, value) {
    const charWidth = 6.5;
    const labelWidth = Math.round(label.length * charWidth + 12);
    const valueWidth = Math.round(value.length * charWidth + 12);
    const width = labelWidth + valueWidth;
    return (`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="20" role="img" aria-label="${escapeXml(`${label}: ${value}`)}">` +
...
export function renderRepoSection(input, today) {
    ...
    lines.push(`| Stars | ${formatInt(stars)}${starsDelta === null ? "" : ` (${starsDelta >= 0 ? "+" : ""}${formatInt(starsDelta)})`} |`);
    lines.push("");
    ...
    for (const row of [...days30].reverse()) {
        lines.push(`| ${row.date} | ${formatInt(row.views)} | ${formatInt(row.views_uniques)} | ${formatInt(row.clones)} | ${formatInt(row.clones_uniques)} | ${formatInt(row.stars)} |`);
    }
    lines.push("");
    lines.push(`### Top referrers${lastReferrerDate ? ` (14 days to ${lastReferrerDate})` : ""}`);
    lines.push("");
    if (referrers.length === 0)
        lines.push("_No referrer data yet._");
    else {
    ...
    lines.push("");
    if (paths.length === 0)
        lines.push("_No path data yet._");
    else {
...
export function renderReport(inputs, today, version) {
