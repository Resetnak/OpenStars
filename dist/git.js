 ...
 */
function git(options, args, cwd = options.dir) {
    ...
    try {
        ...
        throw new Error(`git ${args[0]} failed: ${stderr.replace(basic, "***") || err.message}`);
    }
}
/** Clone the data branch, or start an orphan one when it does not exist yet. Returns true when created. */
export function prepareBranch(options) {
    let created = false;
    try {
...
export function commitAndPush(options, message) {
    git(options, ["add", "--all"]);
    if (git(options, ["status", "--porcelain"]).trim() === "")
        return false;
