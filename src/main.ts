...
import { collect, type Snapshot } from "./collect.js";
import { toCsv } from "./csv.js";
import { commitAndPush, prepareBranch, type GitOptions } from "./git.js";
import { emptyFile, merge, parseFile, windowDates, type TrafficFile } from "./merge.js";
import { badgeSvg, formatInt, lastDays, latest, renderReport, REPOMETER_URL, sparklineSvg, sum, type RepoReportInput } from "./report.js";
...
async function run(): Promise<void> {
  ...
  if (!ownRepo) throw new Error("GITHUB_REPOSITORY is not set; OpenStars must run inside GitHub Actions.");
  const serverUrl = process.env.GITHUB_SERVER_URL ?? "https://github.com";

  const repos = (getInput("repos") || ownRepo).split(/[\s,]+/).filter(Boolean);
  for (const repo of repos) {
    if (!REPO_PATTERN.test(repo)) throw new Error(`"${repo}" is not a valid owner/name repository.`);
  }
...
function writeRepoData(
  ...
  const existing: TrafficFile = existsSync(jsonPath) ? parseFile(readFileSync(jsonPath, "utf8"), snapshot.repo) : emptyFile(snapshot.repo);
...
function firstRunSummary(ownRepo: string, branch: string, files: RepoReportInput[]): string {
  const lines = [
...
function runSummary(files: RepoReportInput[], today: string): string {
  const lines: string[] = [];
...
run().catch((err: Error) => fail(err.message));
